...
export const ClientKeysTab: React.FC = () => {
  ...
  const isPassThrough = (u: any) => Boolean(u?.followUpstream || u?.id === "up_bandelbanget_follow");
  ...
  const handleCreateSecretKey = async (e: React.FormEvent) => {
    ...
    const isPassThroughChecked = newAllowedProviders.some((id) => {
      const up = upstreams.find((u) => u.id === id);
      return up ? isPassThrough(up) : id === "up_bandelbanget_follow";
    });
    ...
    try {
      await apiRequest("/api/keys", {
        ...
        body: JSON.stringify({
          ...
          customKey: isPassThroughChecked ? undefined : (newCustomSecretKey || undefined),
          ...
          isFollowUpstream: isPassThroughChecked,
        }),
      });
  ...
  const handleToggleProviderForActiveKey = async (providerId: string) => {
    ...
    const targetUpstream = upstreams.find((u) => u.id === providerId);
    const isTargetFollow = targetUpstream ? isPassThrough(targetUpstream) : (providerId === "up_bandelbanget_follow" || providerId === "bb");
    ...
    if (willAllow) {
      if (isTargetFollow) {
        ...
        const nonFollow = current.filter((p) => {
          const up = upstreams.find((u) => u.id === p);
          return up ? !isPassThrough(up) : (p !== "up_bandelbanget_follow" && p !== "bb");
        });
        updated = [...nonFollow, providerId];
      }
    } else {
  ...
  const handleToggleAllProvidersForActiveKey = async (allowAll: boolean) => {
    if (!activePermKey) return;
    const isPermPassThrough = (activePermKey.allowedProviders || []).some((id) => {
      const up = upstreams.find((u) => u.id === id);
      return up ? isPassThrough(up) : (id === "up_bandelbanget_follow" || id === "bb");
    });

    const target = allowAll
      ? isPermPassThrough
        ? upstreams.filter(isPassThrough).map((u) => u.id)
        : upstreams.filter((u) => !isPassThrough(u)).map((u) => u.id)
      : [];
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleCreateSecretKey} className="space-y-4 text-xs">
              ...
              {/* Provider Access Selection & Custom Secret Key: Pass-Through Exclusivity */}
              {(() => {
                const isPassThroughChecked = newAllowedProviders.some((id) => {
                  const up = upstreams.find((u) => u.id === id);
                  return up ? isPassThrough(up) : (id === "up_bandelbanget_follow" || id === "bb");
                });
                const displayedUpstreams = isPassThroughChecked
                  ? upstreams.filter(isPassThrough)
                  : upstreams;

                return (
                  <>
                    {!isPassThroughChecked && (
                      <div>
                    ...
                    <div className="p-3.5 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 space-y-2.5">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-400 mt-0.5">
                            {isPassThroughChecked
                              ? "Pass-Through active: Non-pass-through providers hidden (cannot be mixed)."
                        ...
                        <div className="flex items-center space-x-1.5">
                          <button
                            type="button"
                            onClick={() => {
                              if (isPassThroughChecked) {
                                setNewAllowedProviders(upstreams.filter(isPassThrough).map((u) => u.id));
                              } else {
                                setNewAllowedProviders(upstreams.filter((u) => !isPassThrough(u)).map((u) => u.id));
                              }
                            }}
                      ...
                      {isPassThroughChecked && (
                        <div className="p-2.5 rounded-lg bg-purple-500/10 border border-purple-500/30 text-xs text-purple-300 flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse shrink-0" />
                            <span>Pass-Through Mode: Non-pass-through providers hidden (cannot be mixed).</span>
                          </div>
                          <button
                            type="button"
                            onClick={() => setNewAllowedProviders([])}
                            className="text-[10px] font-bold text-purple-400 hover:text-purple-200 underline cursor-pointer shrink-0 ml-2"
                          >
                            Reset / Select Normal
                          </button>
                        </div>
                              ...
                              >
                                <div className="flex items-center space-x-2 min-w-0">
                                  <input
                                    ...
                                    checked={isChecked}
                                    onChange={() => {
                                      const isTargetFollow = isPassThrough(u);
                                      if (isChecked) {
                                        setNewAllowedProviders(newAllowedProviders.filter((id) => id !== u.id));
                                      } else {
                                        if (isTargetFollow) {
                                          setNewCustomSecretKey("");
                                          setNewAllowedProviders([u.id]);
                                        } else {
                                          const nonFollow = newAllowedProviders.filter((id) => {
                                            const up = upstreams.find((x) => x.id === id);
                                            return up ? !isPassThrough(up) : (id !== "up_bandelbanget_follow" && id !== "bb");
                                          });
                                          setNewAllowedProviders([...nonFollow, u.id]);
                                        }
                                      }
                                    }}
                                ...
                                <span className="text-[10px] font-mono text-zinc-400 truncate max-w-[140px]">
                                  {isFollow ? "Direct Pass-Through" : (u.baseUrl || "Official API")}
                                </span>
                              </label>
                            );
                          })
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto border border-zinc-200 dark:border-zinc-800 rounded-lg p-2 space-y-2 max-h-72">
              {(() => {
                ...
                const displayedPermUpstreams = isPermPassThrough
                  ? upstreams.filter(isPassThrough)
                  : upstreams;
                ...
                return (
                  <>
                    {isPermPassThrough && (
                      <div className="p-2.5 rounded-lg bg-purple-500/10 border border-purple-500/30 text-xs text-purple-300 flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse shrink-0" />
                          <span>Pass-Through Mode: Non-pass-through providers hidden (cannot be mixed).</span>
                        </div>
